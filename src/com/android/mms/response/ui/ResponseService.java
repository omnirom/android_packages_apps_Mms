...
import com.android.mms.response.util.LogEntry;

@SuppressWarnings("deprecation")
public class ResponseService extends Service {
    private static final String TAG = "AutoResponseService";
    private static final String logFile = "autoresponse.log";
    ...
    public void onStart(Intent intent, int startId) {
        ...
        sentPI = PendingIntent.getBroadcast(this, 0, new Intent(
                "android.provider.Telephony.SMS_SENT"), 0);
            ...
            smsReceiver = new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
                    ...
                    if (null != bundle) {
                        ...
                        Log.i(TAG, "Text Message has been received from: "
                                + hyphenate(getReturnAddress()));
                        initTextSend();
                    }
                }
            };
        ...
        resend = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, Intent intent) {
                // Resending here WILL DRAIN battery if error is persistent (NO
                // DATA in a Gym Locker etc)
                ...
                case SmsManager.RESULT_ERROR_NO_SERVICE:
                    ...
                    if (noServiceResendAttempts > 5) {
                        Log.i(TAG, "Attempting resend: "
                                + (noServiceResendAttempts++));
                        sendSms();
                    } else {
                        createNotificationFromString(getResources().getString(
                ...
                case SmsManager.RESULT_ERROR_RADIO_OFF:
                    ...
                    createNotificationFromString(getResources().getString(
                            R.string.error_radio_off));
                    break;
                }
            }
        };
        registerReceiver(resend, new IntentFilter(
    ...
    private void initTextSend() {
        if (noRepetition() && notListed()) {
            ...
            Log.i(TAG, "Timer set, waiting "
                    + (getDelayDuration() + phoneOffset) + "s");
            tHandler.postDelayed(tGo,
                    (long) (1000 * (getDelayDuration() + phoneOffset)));
        }
    }
    ...
    private void sendSms() {
        ...
        if (getMessageContent(getInformStatus()).length() > 160) {
            ArrayList<String> messagelist = manager
                    .divideMessage(getMessageContent(getInformStatus()));

            manager.sendMultipartTextMessage(getReturnAddress(), null,
                    messagelist, null, null);
            Log.i(TAG, "Multipart Text Message Sent!");
        } else {
            manager.sendTextMessage(getReturnAddress(), null,
                    getMessageContent(getInformStatus()), sentPI, null);
    ...
    private boolean noRepetition() {
        if (addresses.contains(getReturnAddress())) {
            if (getRepeatStatus())
                ...
                Log.w(TAG, hyphenate(getReturnAddress())
        ...
        if (inFile.exists()) {
            try {
                ...
                while (((line = buffreader.readLine()) != null)) {
                    ...
                    LogEntry newLogEntry = new LogEntry(Integer.parseInt(line),
                            buffreader.readLine(), buffreader.readLine(),
                            buffreader.readLine(), buffreader.readLine());
                ...
                Log.e(TAG, "IOException on \"" + file + "\"", exception);
            }
            ;
        }
    ...
    private void grabNumbers(String file) {
        ...
        if (inFile.exists()) {
            try {
                ...
                Log.e(TAG, "IOException caused by buffreader.readLine()",
                        exception);
            }

            if (numOfFilters == 0) {
                Toast eToast = Toast.makeText(
                        this,
                        getResources().getString(
                                R.string.prompt_filtering_empty),
                        Toast.LENGTH_LONG);
                eToast.show();
            } else
    ...
    private void logAddress() {
        ...
        int year = Integer.parseInt(Integer.toString(c.get(Calendar.YEAR))
                .substring(2, 4));
        int month = c.get(Calendar.MONTH) + 1;
        int day = c.get(Calendar.DAY_OF_MONTH);

        logEntry.setTime(pad(hour) + ":" + pad(minute) + ":" + pad(second)
                + " " + timeOfDay);
    ...
    private void saveLog(String file) {
        try {
            OutputStreamWriter oWriter = new OutputStreamWriter(openFileOutput(
                    file, 0));

            oWriter.write("");
            for (int i = 0; i < entries.size(); i++) {
                oWriter.append(Integer.toString(entries.get(
                        entries.size() - 1 - i).getType())
                        + "\n");
    ...
    private void createNotification() {
        ...
        if (getNotifyCount() > 0) {
            ...
            notification = new Notification(R.drawable.stat_notify_sms,
                    getResources()
                            .getString(R.string.notification_ticker_start),
                    System.currentTimeMillis());
            notification.setLatestEventInfo(this,
                    getResources().getString(R.string.app_name),
                    getMessageContent(false), PendingIntent.getActivity(this,
                            0, new Intent(this,
                                    ResponsePreferenceActivity.class), 0));
            ...
            if (getName().equals(""))
                notification = new Notification(R.drawable.stat_notify_sms,
                        getResources().getString(
                                R.string.notification_ticker_replied)
                                + " " + hyphenate(getReturnAddress()),
                        System.currentTimeMillis());
            else
                notification = new Notification(R.drawable.stat_notify_sms,
            ...
            notification.setLatestEventInfo(this,
                    getResources().getString(R.string.app_name) + " ("
                            + getNotifyCount() + ")", getMessageContent(false),
                    PendingIntent.getActivity(this, 0, new Intent(this,
                            ResponsePreferenceActivity.class), 0));

            Log.i(TAG, "Notification Updated: " + getNotifyCount()
                    + " responses this lifecycle");
        } else {
    ...
    private void createNotificationFromString(String message) {
        ...
        // Service started, first notification
        notification = new Notification(R.drawable.stat_notify_sms, message,
                System.currentTimeMillis());
        notification.setLatestEventInfo(this,
                getResources().getString(R.string.app_name),
                getMessageContent(false), PendingIntent.getActivity(this, 0,
                        new Intent(this, ResponsePreferenceActivity.class), 0));
    ...
    public String hyphenate(String number) {
        ...
        if (number.length() == 11)
            return number.substring(0, 1) + "-" + number.substring(1, 4) + "-"
    ...
    private String dehyphenate(String number) {
        if (number.length() == 12)
            ...
            return number.substring(0, 1) + number.substring(2, 5)
                    + number.substring(6, 9) + number.substring(10, 14);
    ...
    public void setSilentStatus(boolean silentStatus) {
    ...
    public void setCallText(int callText) {
        this.callText = callText;
    }

    public String getMessageContent(boolean status) {
        if (status)
            return "Auto Response: " + messageContent;
    ...
    public void setFile(int id) {
        if (id == FILTER_WHITELIST)
            filterFile = "filtering_whitelist.txt";
        if (id == FILTER_BLACKLIST)
            filterFile = "filtering_blacklist.txt";
    }
    ...
    public boolean getList() {
        if (pNumbers.contains(getReturnAddress()))
            return true;
        else
            return false;
    }
