...
import com.android.mms.response.util.PhoneContact;

public class ContactPicker extends ListActivity {
    private static final String TAG = "ContactPicker_AutoResponse";
    ...
    public void onCreate(Bundle SavedInstanceState) {
        ...
        while (cursor.moveToNext()) {
            ...
            if (num > 0) {
                PhoneContact newPContact = new PhoneContact(
                ...
                random.add(cursor.getString(cursor
                        .getColumnIndexOrThrow(ContactsContract.Contacts.DISPLAY_NAME)));
        }
        ...
        for (int i = 0; i < pContacts.size(); i++) {
            ...
            Cursor phone = getContentResolver().query(
                    ...
                    new String[] { pContacts.get(
                            phoneContactSearch(pContacts, names[i])).getId() },
                    null);
            int index = 0;
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        String name = keyword;
        String search = pContacts.get(phoneContactSearch(pContacts, keyword))
                .getId();
        Cursor phone = getContentResolver().query(
    ...
    public void sortContacts() {
        ...
        for (int i = 0; i < pContacts.size(); i++)
            for (int j = 0; j < pContacts.size() - 1 - i; j++)
                if (pContacts.get(j).getName()
                        .compareTo(pContacts.get(j + 1).getName()) > 0) {
                    String tmpName = pContacts.get(j).getName();
                    String tmpNumber = pContacts.get(j).getNumber();
                    String tmpId = pContacts.get(j).getId();

                    pContacts.get(j).setInfo(pContacts.get(j + 1).getName(),
                            pContacts.get(j + 1).getNumber(),
                            pContacts.get(j + 1).getId());
    ...
    public boolean grabNumbers(String file) {
        ...
        if (inFile.exists()) {
            try {
                ...
                Log.e(TAG, "FileNotFoundException caused by " + getFile(),
                        exception);
    ...
    public void saveNumbers(String file) {
        try {
            ...
            Log.e(TAG, "IOException caused by trying to access " + getFile(),
                    exception);
        }
        ;
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        case android.R.id.home:
            Intent parentActivityIntent = new Intent(this, FilterActivity.class);
            parentActivityIntent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP
                    | Intent.FLAG_ACTIVITY_NEW_TASK);
            parentActivityIntent.putExtra("extraFilterStatus",
                    getFilterStatus());
