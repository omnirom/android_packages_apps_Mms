...
import com.android.mms.response.util.Contact;
import com.android.mms.response.util.MessageListArrayAdapter;
...
public class FilterActivity extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		MessageListArrayAdapter adapter = new MessageListArrayAdapter(this,
				sNames, sNumbers);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.filtering_contact_addCustom:
			...
			Button pButton = (Button) dialog
					.findViewById(R.id.dialog_filteringButtonPositive_add);
			pButton.setOnClickListener(new View.OnClickListener() {
				public void onClick(View v) {
					EditText eName = (EditText) dialog
							.findViewById(R.id.dialog_filteringNameEdit_add);
					EditText eNumber = (EditText) dialog
							.findViewById(R.id.dialog_filteringNumberEdit_add);

					if (eName.getText().toString().equals("")
							|| eName.getText().toString().equals(null)
							|| eNumber.getText().toString().equals("")
							|| eNumber.getText().toString().equals(null))
						...
						if (numberExists(hyphenate(eNumber.getText().toString())))
							...
							Contact newContact = new Contact(eName.getText()
									.toString().trim(), hyphenate(eNumber
									.getText().toString().trim()));
							contacts.add(newContact);
							showTheMessage(FILTERING_ADDED, eName.getText()
									.toString().trim());
							dialog.cancel();
							sortNames();
							saveNumbers(getFile());
			...
			Button nButton = (Button) dialog
					.findViewById(R.id.dialog_filteringButtonNegative_add);
		...
		case android.R.id.home:
			Intent parentActivityIntent = new Intent(this, ResponsePreferenceActivity.class);
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		case CONTEXT_MENU_EDIT:
			...
			dialog.setTitle(r.getString(R.string.menu_edit) + " "
					+ contacts.get(iId).getName());

			Button pButton = (Button) dialog
					.findViewById(R.id.dialog_filteringButtonPositive_edit);
			pButton.setOnClickListener(new View.OnClickListener() {
				public void onClick(View v) {
					EditText eName = (EditText) dialog
							.findViewById(R.id.dialog_filteringNameEdit_edit);
					EditText eNumber = (EditText) dialog
							.findViewById(R.id.dialog_filteringNumberEdit_edit);
					...
					if (eName.getText().toString().equals("")
						...
						if ((contacts.get(iId).getName().equals(eName.getText()
								.toString()))
								&& (contacts.get(iId).getNumber()
										.equals(hyphenate(eNumber.getText()
												.toString()))))
							dialog.cancel();

						else {
							contacts.get(iId).setInfo(
									eName.getText().toString().trim(),
									hyphenate(eNumber.getText().toString()
											.trim()));
							showTheMessage(FILTERING_SAVED, eName.getText()
									.toString().trim());
							dialog.cancel();
							saveNumbers(getFile());

							Log.i(TAG, "\"" + contacts.get(iId).getName()
									+ "\" edited successfully");
							startActivity(getIntent());
							finish();
						}
			...
			Button nButton = (Button) dialog
					.findViewById(R.id.dialog_filteringButtonNegative_edit);
			nButton.setOnClickListener(new View.OnClickListener() {
				public void onClick(View v) {
					dialog.cancel();
				}
			...
			EditText eTitle = (EditText) dialog
					.findViewById(R.id.dialog_filteringNameEdit_edit);
			eTitle.setText(contacts.get(iId).getName());
			EditText eContent = (EditText) dialog
					.findViewById(R.id.dialog_filteringNumberEdit_edit);
		...
		try {
			...
			if (inFile.exists()) {
				...
				while ((line = bReader.readLine()) != null) {
					Contact contactFromFile = new Contact(line,
							bReader.readLine());
	...
	public void saveNumbers(String file) {
		try {
			...
			Log.e(TAG, "IOException caused by trying to access " + file,
					exception);
		}
		;
	}
	...
	public void showTheMessage(int id, String extra) {
		...
		case FILTERING_ADDED:
			message = "\'" + extra + "\'" + " "
					+ r.getString(R.string.prompt_added);
			break;
		...
		case FILTERING_SAVED:
			message = "\'" + extra + "\'" + " "
					+ r.getString(R.string.prompt_message_saved);
			break;
		}
		...
		for (int i = 0; i < contacts.size(); i++)
			for (int j = 0; j < contacts.size() - 1 - i; j++)
				if (contacts.get(j).getName()
					...
					contacts.get(j).setInfo(contacts.get(j + 1).getName(),
							contacts.get(j + 1).getNumber());
